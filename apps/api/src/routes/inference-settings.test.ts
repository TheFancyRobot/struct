...
describe('inference settings route', () => {
  it('keeps credential references write-only and rejects incompatible assignments', async () => {
    ...
    const provider = await route(new Request('http://local/api/settings/inference/providers', { method: 'POST', body: JSON.stringify({ type: 'OpenAI', credentialReference: 'secret://openai' }) }))
    expect(provider.status).toBe(201)
    const listed = await route(new Request('http://local/api/settings/inference'))
    expect(await listed.text()).not.toContain('secret://openai')
