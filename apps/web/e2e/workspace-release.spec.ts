...
import { startRealAppStack, stopRealAppStack } from './support/app-server'
import { isExpectedRequestAbort, waitForNoteSaveAndRefresh } from './support/note-save'

const uuidPattern = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i
const folderFixture = resolve(import.meta.dir, 'fixtures/release-folder')
const sourceReadinessTimeoutMs = 60_000
const releaseJourneyTimeoutMs = 180_000
...
function readinessTimeoutWithin(deadline: number, waitingFor: string): number {
  const remaining = deadline - Date.now()
  if (remaining <= 0) {
    throw new Error(
      `Release journey shared ${releaseJourneyTimeoutMs / 1_000}-second budget exhausted before waiting for ${waitingFor}.`,
    )
  }
  return Math.min(sourceReadinessTimeoutMs, remaining)
}

async function waitForReleaseJourneyReadiness(
  deadline: number,
  waitingFor: string,
  waitFor: (timeout: number) => Promise<void>,
): Promise<void> {
  try {
    await waitFor(readinessTimeoutWithin(deadline, waitingFor))
  } catch (error) {
    if (deadline - Date.now() <= 0) {
      throw new Error(
        `Release journey shared ${releaseJourneyTimeoutMs / 1_000}-second budget exhausted while waiting for ${waitingFor}.`,
        { cause: error },
      )
    }
    throw error
...
async function runReleaseJourney(
  ...
  try {
    ...
    const waitForReadySource = async (name: string | RegExp) => {
      ...
      await waitForReleaseJourneyReadiness(deadline, `source ${String(name)} to become ready`, (timeout) =>
        item.getByText('ready', { exact: true }).waitFor({ timeout }),
      )
    }
    const waitForSourceCheckbox = async (name: string | RegExp) => {
      await waitForReleaseJourneyReadiness(deadline, `source checkbox ${String(name)}`, (timeout) =>
        page.getByRole('checkbox', { name }).waitFor({ timeout }),
      )
    }
    ...
    await waitForSourceCheckbox(sourceName)
    ...
    await page.waitForURL(
      new RegExp(`/projects/${projectId}/research/${uuidPattern.source}/runs/${uuidPattern.source}$`, 'i'),
    )
    ...
    expect(await page.locator('#evidence-heading').evaluate((element) => element === document.activeElement)).toBe(true)
    ...
    const noteId = requireMatch(page.url(), /\/notes\/([0-9a-f-]{36})$/i, 'note id missing from note url')[1]!
    ...
    await Promise.race([datasetAnswer.waitFor(), unavailable.waitFor()])
    ...
    await page.getByText('SELECT COUNT(*) AS row_count FROM "records" ORDER BY ALL').waitFor()
...
it(
  'takes a first-time user through root and BASE_PATH durable source-grounded workspaces',
  async () => {
    const deadline = Date.now() + releaseJourneyTimeoutMs
      ...
      try {
        ...
        try {
          for (const scenario of releaseJourneyScenarios) {
            ...
            try {
              stack = await withProcessEnvironment(scenario.environment, () => startRealAppStack(scenario.port))
              await runReleaseJourney(page, scenario, deadline)
            } finally {
