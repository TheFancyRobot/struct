...
export const createInferenceProvider = (input: { type: '@fancyrobot/fred-openai', endpoint?: string, credentialReference: string }) => request('/settings/inference/providers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
export const updateInferenceProvider = (id: string, input: { type: '@fancyrobot/fred-openai', endpoint?: string, credentialReference?: string }) => request(`/settings/inference/providers/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
...
export const clearInferenceModelAssignment = (role: InferenceRole) => request(`/settings/inference/assignments/${role}`, { method: 'DELETE' })
