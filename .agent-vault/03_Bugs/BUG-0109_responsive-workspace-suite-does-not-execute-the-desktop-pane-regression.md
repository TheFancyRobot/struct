...
- 2026-08-04: Addressed the PR #143 automated-review finding in `apps/web/e2e/workspace-responsive.spec.ts`: the routed-project-error `waitForResponse` predicate now parses each response URL and matches a pathname ending in `/api/projects`, so it works for both root and non-root `BASE_URL` deployments (for example, `/struct/api/projects`).
- Verification: `cd apps/web && bun test --timeout 60000 --max-concurrency 1 ./e2e/workspace-responsive.spec.ts` — 9 passing tests, 0 failures, including `collapses and restores desktop panes independently`.
