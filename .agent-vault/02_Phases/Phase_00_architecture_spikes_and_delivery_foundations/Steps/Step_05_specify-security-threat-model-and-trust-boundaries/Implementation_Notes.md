...
- **STEP-00-03 (historical DuckDB topology spike evidence)** — selected the
  `worker` child-process candidate at the time; `direct` was rejected because
  a native crash was not containable. Preserve the measured JSON-over-stdio,
  hardening order, `memory_limit` ~244 MiB, `threads=2`, interruption/timeout
  behavior, atomic Parquet promotion, byte cap, and denial probes. The
  host-child topology itself is superseded by DEC-0003/DEC-0005: Phase 04
  implements an isolated sidecar, with no DuckDB child/native adapter in the
  maintained Bun host.
...
- Cross-doc contradiction check: §19 boundaries match architecture.md §3.2 exactly; DEC-0009 (registered roots + allowlist SQL) and STEP-00-03's still-applicable hardening order, DENIED probes, and selected-at-the-time worker measurements are consistent and cited. DEC-0003/DEC-0005 supersede the historical host-child topology with the pinned Phase-04 sidecar. No contradictions.
