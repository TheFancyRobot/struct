...
- Added `packages/retrieval` for workspace/project/source-version-scoped deterministic FTS and stable line locators, `packages/research-engine` for fixed typed plan/result and evidence/citation checks, and `packages/workflows` for the bounded Fred graph.
