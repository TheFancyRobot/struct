...
- Added `services/data-engine-sidecar` with a pinned Node `24.18.0` runtime (verified upstream SHA-256 per arm64/amd64) and `@duckdb/node-api` `1.5.4-r.1`. The service exposes only authenticated version-1 materialization and artifact-read operations; it has no arbitrary SQL endpoint.
