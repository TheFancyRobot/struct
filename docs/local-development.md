...
| `API_WORKSPACE_ID` | `apps/api`, `apps/web` build | workspace UUID owned by the configured bearer identity; compiled as a public browser routing scope, never a credential | local configuration |
