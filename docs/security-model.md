...
- expose only versioned materialization and parser-allowlisted read-only query
  operations; never expose unrestricted raw SQL;
...
| 5. Model | bounded context (system/developer instructions, tool results, untrusted source excerpts); model proposals/synthesis | Fred agents (planner, SQL planner, corpus analyst, evidence critic, synthesizer); model-generated unsafe actions | `packages/workflows` tool gates (tool authorization re-checks) + `packages/research-engine` citation/synthesis gates | `packages/workflows` (tool gates, prompts); `packages/research-engine` (evidence/citation gates) | tools re-check workspace/project/source permissions independently of prompts (§6.3); models never receive raw authority; source content framed as evidence; budgets/cancellation enforced outside prompts | `tool.invocation`, `tool.authorization_failure`, `citation.validation_failure`, `budget.exceeded`, `plan.revised` | typed tool/citation failure; fail closed on schema mismatch; no broadened permissions |
| 6. Data-engine | dataset catalogs, schema profiles, typed materialization/query requests/results, Parquet files | worker submits catalog-derived materialization requests; SQL planner proposes read-only queries | `packages/data-engine` typed authenticated client and SQL policy; container hardening and engine policy inside `services/data-engine-sidecar` | `packages/data-engine` (protocol/policy); Compose/service owner (container lifecycle) | version-1 materialization and allowlisted read-only query operations; derived content-addressed paths; catalog-authorized immutable snapshot bindings; total ordering; approved mounts only; no egress/Docker socket; time/memory/CPU/process/row/byte/concurrency limits; cancellation | `query.validation_failure`, `query.cancelled`, `query.resource_limit`, `parquet.promote`, `sidecar.restart` | typed protocol/transport/resource failure; no partial artifact promotion; sidecar crash cannot terminate the Bun worker; durable attempt ownership fences late completion |
...
| THR-11 | Model | EoP / Tampering | permissions, citation policy, boundaries | prompt-injected agent | untrusted source loaded | injection across supported source types | authority broadening / citation skip / code exec | prompt isolation + tool authz re-checks + no secrets in context + bounded context (`packages/workflows`) | `tool.authorization_failure`, `citation.validation_failure`; injection eval suite | low | controlled | Phase 04 | nightly + Phase 09 |
| THR-12 | Model | Info disclosure | secrets, other-workspace data | model/tool | model context assembled | exfiltration via prompt/tool request | secret/data leak | redaction + workspace-scoped tool authz + no secrets in context (`packages/workflows`) | `tool.authorization_failure` | low | controlled | Phase 04 | Phase 04+ |
...
| Max model calls per run | TBD | `packages/workflows` | corpus eval | terminate | Phase 05 |
| Max tool calls per run | TBD | `packages/workflows` | corpus eval | terminate | Phase 05 |
| Max tokens per run | TBD | `packages/workflows` | corpus eval | terminate | Phase 05 |
...
| ABUSE-06 | adversarial (nightly) | prompt-injection suite across every supported source type; asserts no broadened perms, no citation skip, no exfiltration | `packages/evaluation` + `workflows` | Phase 04 / nightly |
