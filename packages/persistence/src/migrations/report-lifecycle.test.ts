...
describe('report lifecycle migration contract', () => {
  it('persists canonical scope, revisions, evidence, and section links', async () => {
    ...
    expect(sql).toMatch(/CREATE TABLE report_revision_snapshots/i)
    expect(sql).toMatch(/CREATE TABLE report_revision_source_versions/i)
    expect(sql).toMatch(/CREATE TABLE report_revision_sections/i)
    expect(sql).toMatch(/CREATE TABLE report_revision_claims/i)
    expect(sql).toMatch(/UNIQUE \(report_id, report_revision, claim_id\)/i)
  ...
  it('drops the direct greenfield shape in dependency order', async () => {
    ...
    expect(sql.indexOf('DROP TABLE IF EXISTS report_revision_claims'))
      .toBeLessThan(sql.indexOf('DROP TABLE IF EXISTS reports'))
    expect(sql.indexOf('DROP TABLE IF EXISTS report_revision_source_versions'))
      .toBeLessThan(sql.indexOf('DROP TABLE IF EXISTS reports'))
